...
/*   Updated: 2023/05/24 18:52:32 by oaboulgh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_cmd(t_tree *root, int infile_fd, int outfile_fd, t_env **env)
{
	...
	if (root->token && root->token->type == PIPE && pipe_handle(root, &data, env))
		return (1);
	else if (root->red && root->red->token && root->red->token->type == RED_OUT && red_out_hanlde(root, data, env))
		return (1);
	else if (root->token && root->token->type == D_RED_OUT && handle_append(root, &data, env))
		return (1);
	else if (root->token && root->token->type == RED_IN && handle_red_in(root, &data, env))
		return (1);
		...
		return (1);
	else if (root->token && root->token->type == D_RED_IN && handle_here_doc(root, &data, env))
		return (1);
